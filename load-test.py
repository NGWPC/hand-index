...
class DatabaseLoader:
        ...
        with self.engine.begin() as conn:
            for version in versions:
                conn.execute(
                    ...
                    VALUES (:hand_version_id, :release_date, :description)
                """
                    ),
                    version,
                )
    ...
    def load_nwm_features(self, gpkg_path):
        ...
        with self.engine.begin() as conn:
            for _, row in gdf.iterrows():
                ...
                # If no row was returned, it means there was a conflict and the insert was skipped
                if result.rowcount == 0:
                    dropped_count += 1
                    print(f"\nDropped duplicate feature:")
                    print(f"  Feature ID: {row['ID']}")
                    print(f"  To Feature: {row['to']}")
                    print(f"  Stream Order: {row['order_']}")
                    print(f"  Lake: {row['Lake']}")
                    print(f"  Gages: {row['gages']}")
                    print(f"  Slope: {row['Slope']}")
                    print(f"  Mainstem: {row['mainstem']}")

        if dropped_count > 0:
            print(f"\nTotal dropped features: {dropped_count}")
    ...
    def load_catchments(self, catchment_dir):
        ...
        for subdir in os.listdir(catchment_dir):
            ...
            for gpkg_file in gpkg_files:
                gpkg_path = os.path.join(subdir_path, gpkg_file)

                # Clean up any WAL mode settings
                try:
                    ...
                    print(f"Warning: Could not clean up WAL for {gpkg_file}: {str(e)}")
                ...
                try:
                    ...
                    with self.engine.begin() as conn:
                        ...
                        """
                            ...
                            {"id": catchment_id, "geom": merged_geom.wkt},
                        )
                except Exception as e:
                    print(f"Warning: Could not process {gpkg_file}: {str(e)}")
                    continue
    ...
    def load_hand_rem_rasters(self, catchment_dir, catchment_ids):
        ...
        for subdir, catchment_id in catchment_ids.items():
            subdir_path = os.path.join(catchment_dir, subdir)
            rem_files = [
                f for f in os.listdir(subdir_path) if f.startswith("rem_zeroed_masked_")
            ]
    ...
    def load_hand_catchment_rasters(self, catchment_dir, catchment_ids):
        ...
        for subdir, catchment_id in catchment_ids.items():
            ...
            catchment_files = [
                ...
                for f in os.listdir(subdir_path)
                if f.endswith(".tif")
                and "gw_catchments_reaches_filtered_addedAttributes_" in f
            ]
    ...
    def load_hydrotables(self, catchment_dir, catchment_ids):
        ...
        for subdir, catchment_id in catchment_ids.items():
            subdir_path = os.path.join(catchment_dir, subdir)
            hydro_files = [
                f for f in os.listdir(subdir_path) if f.startswith("hydroTable_")
            ]
